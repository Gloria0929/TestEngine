<template>
  <div class="topbar">
    <div class="left">
      <el-popover v-model:visible="popoverVisible" trigger="click" placement="bottom-start" :width="200" :offset="4"
        :show-arrow="false">
        <template #reference>
          <el-input :model-value="currentProjectName" readonly size="small" style="width: 200px" class="project-input">
            <template #suffix>
        ...
        <div class="project-popover">
          ...
          <div v-for="p in projects" :key="p.id" class="popover-item" :class="{ active: p.id === currentProjectId }"
    ...
    <div class="right">
      <el-badge :value="notification.unreadCount" :hidden="notification.unreadCount === 0">
        <el-icon class="action" @click="showNotif = true">
          <Bell />
        </el-icon>
      </el-badge>
      ...
      <el-dropdown @command="onUser">
        <span class="user"><el-avatar :size="26">{{
    ...
    <el-drawer v-model="showNotif" title="通知" size="360px">
      ...
      <div v-for="n in notification.list" :key="n.id" class="notif-item" :class="{ unread: !n.read }"
        @click="notification.markOne(n.id)">
        <div class="title">{{ n.title }}</div>
        <div class="content">{{ n.content }}</div>
      </div>
      <el-button v-if="notification.unreadCount > 0" size="small" @click="notification.markAll()">全部已读</el-button>
    </el-drawer>
    ...
    <el-dialog v-model="addVisible" title="添加项目" width="420px">
      <el-form :model="addForm" label-width="72px">
        ...
        <el-form-item label="描述">
          <el-input v-model="addForm.description" type="textarea" :rows="3" placeholder="请输入项目描述" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="addVisible = false">取消</el-button>
        <el-button type="primary" :loading="adding" @click="onAddProject">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function onUser(cmd: string) {
  if (cmd === "logout") userStore.logout().then(() => router.replace("/"));
