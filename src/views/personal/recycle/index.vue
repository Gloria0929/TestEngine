          ...
          <el-table-column label="操作" min-width="180" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" @click="onRestoreCase(row)"
                >恢复</el-button
              >
              <el-button link type="danger" @click="onPurgeCase(row)"
                >彻底删除</el-button
              >
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" min-width="180" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" @click="onRestoreScenario(row)"
                >恢复</el-button
              >
              <el-button link type="danger" @click="onPurgeScenario(row)"
                >彻底删除</el-button
              >
            </template>
          </el-table-column>
...
function caseStatusText(s: TestCase["status"]) {
...
async function onPurgeCase(row: TestCase) {
  await ElMessageBox.confirm("彻底删除后不可恢复，确认删除？", "确认", {
    type: "warning",
  });
