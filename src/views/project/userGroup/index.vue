<template>
  <div class="user-group">
    ...
    <el-drawer v-model="drawerVisible" :title="current ? `配置权限：${current.name}` : '配置权限'" size="360px">
      <PermissionTree v-if="current" :data="tree" :model-value="current ? current.permissions : []" @update:model-value="onPermissionChange" />
      <template #footer>
...
import { fetchGroups, fetchPermissionTree, updateGroupPermissions } from '@/api/project'
...
async function onSave() {
  ...
  await updateGroupPermissions(current.value.id, current.value.permissions)
