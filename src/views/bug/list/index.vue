<template>
  <div class="bug-list">
    <div class="toolbar">
      ...
      >
        <el-option
          v-for="s in bugSeverities"
          :key="s"
          :label="severityMap[s].label"
          :value="s"
        />
      </el-select>
      <el-input
    ...
    >
      ...
      <template #col-severity="{ row }">
        <el-tag
          :color="severityMap[row.severity as BugSeverity].color"
          size="small"
          >{{ severityMap[row.severity as BugSeverity].label }}</el-tag
        >
      </template>
      <template #col-status="{ row }">
        <el-tag :type="statusMap[row.status as BugStatus].type" size="small">{{
          statusMap[row.status as BugStatus].label
        }}</el-tag>
      </template>
      ...
      <template #actions="{ row }">
        ...
        <el-dropdown trigger="click" @command="onTransition(row, $event)">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item
                v-for="s in transitions(row.status)"
                :key="s"
                :command="s"
                >{{ statusMap[s].label }}</el-dropdown-item
              >
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      ...
      <template v-if="current">
        <el-descriptions :column="1" border>
          <el-descriptions-item label="严重程度">
            <el-tag :color="severityMap[current.severity].color" size="small">{{
              severityMap[current.severity].label
            }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="状态">
            <el-tag :type="statusMap[current.status].type" size="small">{{
        ...
        <div class="comments">
          ...
          <div
            v-for="(c, i) in commentList(current.id)"
            :key="i"
            class="comment-item"
          >
          ...
          <el-empty
            v-if="!commentList(current.id).length"
            description="暂无评论"
            :image-size="60"
          />
          <div class="comment-input">
            <el-input
              ...
              placeholder="添加评论"
            />
            <el-button type="primary" size="small" @click="onAddComment"
              >发表</el-button
            >
          </div>
        </div>
...
const bugSeverities: BugSeverity[] = [
  "BLOCKER",
  "CRITICAL",
  "MAJOR",
  "MINOR",
...
function transitions(s: BugStatus) {
  return bugStatuses.filter((x) => x !== s);
}
...
function onAddComment() {
  ...
  commentList(current.value.id).push({
    user: "Administrator",
    content: text,
