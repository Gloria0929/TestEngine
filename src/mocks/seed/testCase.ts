...
export function createCases(): TestCase[] {
  return [
    { id: 'c-1', projectId: 'p-1', moduleId: 'm-1-1', name: '正确账号密码登录成功', testPoint: '登录功能', precondition: '用户已登录且账户余额充足', steps: [{ id: 's-1', description: '准备测试环境', expected: '环境准备就绪' }, { id: 's-2', description: '输入边界值执行用户完成支付订单，购买系统增值服务', expected: '系统正确处理边界值' }, { id: 's-3', description: '验证边界条件结果', expected: '边界条件下系统行为稳定' }], level: 'P0', status: 'READY', executor: 'test', tags: ['冒烟', '登录'], createUser: 'test', updateTime: '2026-08-26 14:00', follow: true, purpose: '验证账号密码登录功能在正常场景下的可用性，确保合法用户可顺利进入系统。', preCaseIds: ['c-2'], postCaseIds: ['c-5'], relatedCaseIds: ['c-2', 'c-3'], bugIds: ['b-1'] },
