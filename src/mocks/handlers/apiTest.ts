...
function parseCurl(text: string): DebugRequest {
  ...
  for (const raw of tokens) {
    ...
    if (pendingFlag) {
      if (pendingFlag === "header") {
        const idx = t.indexOf(":");
        headers.push({
          key: idx >= 0 ? t.slice(0, idx) : t,
          value: idx >= 0 ? t.slice(idx + 1).trim() : "",
          enabled: true,
        });
  ...
  return {
    id: "",
    name: "导入请求",
    method: (method || (hasData ? "POST" : "GET")) as HttpMethod,
    url,
    protocol: "HTTP",
    headers,
    query: [],
...
export const apiTestHandlers = [
  ...
  http.get("/api/api-test/scenarios/recycle", () =>
    HttpResponse.json(ok(scenarioRecycle)),
  ),
